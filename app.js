...
function renderDesignStudio() {
  ...
  $$('[data-starter]').forEach(b=>b.onclick=()=>{snapshot();const im=new Image();im.onload=()=>{ctx.globalCompositeOperation="source-over";ctx.clearRect(0,0,300,300);const scale=Math.min(270/im.width,270/im.height),w=im.width*scale,h=im.height*scale;ctx.drawImage(im,(300-w)/2,(300-h)/2,w,h);hasInk=true;hideHint();toast("精致底稿放好啦，可以给白白加装饰")};im.src=b.dataset.starter});
