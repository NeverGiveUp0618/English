...
function renderDesignStudio() {
  ...
  const canvas=$("#characterCanvas"),ctx=canvas.getContext("2d"),history=[],redo=[];let drawing=false,color="#7b4b2a",size=3,tool="free",filled=false,erasing=false,hasInk=false,start=null,shapeBase=null;
  const hideHint=()=>{if($("#drawHint"))$("#drawHint").style.display="none"};
  const snapshot=()=>{history.push(ctx.getImageData(0,0,300,300));if(history.length>20)history.shift();redo.length=0};
  const toolTip=()=>{const names={free:"曲线",line:"直线",arrow:"箭头",circle:"圆形",rect:"矩形",triangle:"三角形",diamond:"菱形",pentagon:"五边形",hexagon:"六边形",star:"五角星",heart:"爱心",cloud:"云朵",bubble:"对话框"};$("#drawTip").textContent=erasing?"现在是：橡皮 · 在画错的地方轻轻擦":"现在是："+(size===3?"细铅笔":size===6?"彩色笔":"粗画笔")+"画"+names[tool]+(filled&&!["free","line","arrow"].includes(tool)?"（实心）":"")+" · 按住拖动就能画"};
  if(S.designDraft&&S.designDraft.canvas){const im=new Image();im.onload=()=>{ctx.drawImage(im,0,0,300,300);hasInk=true;hideHint()};im.src=S.designDraft.canvas;}
  ...
  const begin=e=>{snapshot();drawing=true;hasInk=true;hideHint();start=point(e);shapeBase=ctx.getImageData(0,0,300,300);ctx.globalCompositeOperation=erasing?"destination-out":"source-over";ctx.strokeStyle=color;ctx.fillStyle=color;ctx.lineWidth=erasing?18:size;ctx.lineCap="round";ctx.lineJoin="round";if(erasing||tool==="free"){ctx.beginPath();ctx.arc(start.x,start.y,ctx.lineWidth/2,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.moveTo(start.x,start.y)}e.preventDefault()};
  const move=e=>{if(!drawing)return;const p=point(e);if(erasing||tool==="free"){ctx.lineTo(p.x,p.y);ctx.stroke()}else{ctx.putImageData(shapeBase,0,0);drawShape(p)}e.preventDefault()};
  ...
  const chooseColor=(c,b)=>{color=c;erasing=false;$$(".drawColor").forEach(x=>x.classList.toggle("on",x===b));$("#drawEraser").classList.remove("on");toolTip()};
  $$(".drawColor").forEach(b=>b.onclick=()=>chooseColor(b.dataset.color,b));
  $("#drawCustomColor").oninput=e=>chooseColor(e.target.value,null);
  $$(".drawSize").forEach(b=>b.onclick=()=>{size=+b.dataset.size;erasing=false;$$(".drawSize").forEach(x=>x.classList.toggle("on",x===b));$("#drawEraser").classList.remove("on");toolTip()});
  $("#drawEraser").onclick=()=>{erasing=true;$("#drawEraser").classList.add("on");toolTip()};
  ...
  $$('[data-starter]').forEach(b=>b.onclick=()=>{snapshot();const im=new Image();im.onload=()=>{ctx.globalCompositeOperation="source-over";ctx.clearRect(0,0,300,300);const scale=Math.min(170/im.width,170/im.height),w=im.width*scale,h=im.height*scale;ctx.drawImage(im,(300-w)/2,112+(170-h)/2,w,h);hasInk=true;hideHint();toast("白白底稿放好啦，四周都留给你创作")};im.src=b.dataset.starter});
  $("#drawUndo").onclick=()=>{const x=history.pop();if(x){redo.push(ctx.getImageData(0,0,300,300));ctx.globalCompositeOperation="source-over";ctx.putImageData(x,0,0)}};$("#drawRedo").onclick=()=>{const x=redo.pop();if(x){history.push(ctx.getImageData(0,0,300,300));ctx.putImageData(x,0,0)}};$("#drawClear").onclick=()=>{snapshot();ctx.clearRect(0,0,300,300);hasInk=false;$("#drawHint").style.display="block";toast("画板清空啦，随时可以重新画")};
  $("#saveDrawing").onclick=()=>{if(!hasInk){toast("先画一点内容再保存吧～");return;}S.designDrawings=S.designDrawings||[];S.designDrawings.push({date:todayStr(),name:$("#roleName").value.trim()||"我的作品",art:canvas.toDataURL("image/png")});if(S.designDrawings.length>12)S.designDrawings.shift();save();sndWin();toast("💾 已保存到我的作品，不会替换白白")};
