...
function renderDesignStudio() {
  ...
  const drawShape=(p)=>{ctx.beginPath();if(tool==="line"){ctx.moveTo(start.x,start.y);ctx.lineTo(p.x,p.y)}else if(tool==="rect"){ctx.rect(Math.min(start.x,p.x),Math.min(start.y,p.y),Math.abs(p.x-start.x),Math.abs(p.y-start.y))}else if(tool==="circle"){ctx.ellipse((start.x+p.x)/2,(start.y+p.y)/2,Math.abs(p.x-start.x)/2,Math.abs(p.y-start.y)/2,0,0,Math.PI*2)}else if(tool==="star"){const cx=(start.x+p.x)/2,cy=(start.y+p.y)/2,rx=Math.abs(p.x-start.x)/2,ry=Math.abs(p.y-start.y)/2;for(let i=0;i<10;i++){const a=-Math.PI/2+i*Math.PI/5,r=i%2?0.42:1,x=cx+Math.cos(a)*rx*r,y=cy+Math.sin(a)*ry*r;i?ctx.lineTo(x,y):ctx.moveTo(x,y)}ctx.closePath()}ctx.stroke()};
  ...
  $("#drawStarter").onclick=()=>{snapshot();hideHint();ctx.globalCompositeOperation="source-over";ctx.strokeStyle="#b88a67";ctx.lineWidth=3;ctx.lineCap="round";ctx.lineJoin="round";ctx.beginPath();ctx.arc(150,105,55,0,Math.PI*2);ctx.moveTo(112,70);ctx.quadraticCurveTo(73,50,84,112);ctx.moveTo(188,70);ctx.quadraticCurveTo(227,50,216,112);ctx.moveTo(122,102);ctx.arc(120,102,3,0,Math.PI*2);ctx.moveTo(183,102);ctx.arc(180,102,3,0,Math.PI*2);ctx.moveTo(150,112);ctx.arc(150,112,5,0,Math.PI*2);ctx.moveTo(117,160);ctx.quadraticCurveTo(150,135,183,160);ctx.quadraticCurveTo(211,195,196,246);ctx.moveTo(117,160);ctx.quadraticCurveTo(89,195,104,246);ctx.moveTo(104,246);ctx.quadraticCurveTo(150,266,196,246);ctx.stroke();hasInk=true;toast("有开头啦！给它加颜色和表情吧");};
  $("#drawUndo").onclick=()=>{const x=history.pop();if(x){ctx.globalCompositeOperation="source-over";ctx.putImageData(x,0,0)}};$("#drawClear").onclick=()=>{snapshot();ctx.clearRect(0,0,300,300);hasInk=false;$("#drawHint").style.display="block";toast("画板清空啦，随时可以重新画")};
