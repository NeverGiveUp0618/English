...
/* ================= 白白设计工坊：装扮涂色 + 自创角色画板 ================= */
const DESIGN_SHAPES = {
  cape:{n:"童话开襟披风",cat:"我的设计",group:"body",base:.92,pos:{x:50,y:62,s:1,r:0}},
  hat:{n:"星光贝雷帽",cat:"我的设计",group:"head",base:.42,pos:{x:50,y:17,s:1,r:0}},
  collar:{n:"蝴蝶结项饰",cat:"我的设计",group:"neck",base:.42,pos:{x:50,y:48,s:1,r:0}}
};
function designSvg(kind, main, trim, mark) {
  const motif = mark === "star" ? "★" : mark === "heart" ? "♥" : mark === "flower" ? "✿" : "";
  let art = "";
  if (kind === "hat") art = `<ellipse cx="150" cy="151" rx="96" ry="24" fill="${trim}" opacity=".95"/><path d="M73 142 Q78 63 150 54 Q222 63 227 142 Q184 164 150 157 Q116 164 73 142Z" fill="${main}" stroke="${trim}" stroke-width="8"/><path d="M150 55 Q172 29 190 50 Q172 68 150 66Z" fill="${trim}"/><circle cx="150" cy="58" r="9" fill="${trim}"/>`;
  else if (kind === "collar") art = `<path d="M145 130 Q92 78 52 106 Q38 143 103 171 Q126 178 148 151Z" fill="${main}" stroke="${trim}" stroke-width="8"/><path d="M155 130 Q208 78 248 106 Q262 143 197 171 Q174 178 152 151Z" fill="${main}" stroke="${trim}" stroke-width="8"/><circle cx="150" cy="142" r="24" fill="${trim}"/><path d="M137 158 L116 218 L150 196 L184 218 L163 158Z" fill="${main}" stroke="${trim}" stroke-width="7"/>`;
  else art = `<path d="M128 74 Q88 79 53 110 Q31 157 24 256 Q62 279 103 245 Q123 223 136 171 Q117 142 128 74Z" fill="${main}" stroke="${trim}" stroke-width="10"/><path d="M172 74 Q212 79 247 110 Q269 157 276 256 Q238 279 197 245 Q177 223 164 171 Q183 142 172 74Z" fill="${main}" stroke="${trim}" stroke-width="10"/><path d="M105 85 Q150 55 195 85" fill="none" stroke="${trim}" stroke-width="18" stroke-linecap="round"/><circle cx="150" cy="83" r="17" fill="${trim}"/>`;
  const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 300"><defs><filter id="s"><feDropShadow dx="0" dy="5" stdDeviation="5" flood-opacity=".22"/></filter></defs><g filter="url(#s)">${art}</g>${motif?`<text x="150" y="${kind==='hat'?126:kind==='collar'?153:kind==='cape'?119:205}" text-anchor="middle" font-size="${kind==='cape'?28:38}" fill="${kind==='cape'?main:trim}" stroke="${kind==='cape'?trim:'none'}" stroke-width="2" font-family="sans-serif">${motif}</text>`:""}</svg>`;
...
function renderCardStudio(left) {
  ...
  const bgs=[["蜜桃云","#ffe3ed","#e5dcff"],["薄荷岛","#dff8ed","#dff0ff"],["星夜","#453c78","#9d79c8"],["暖阳","#fff0b8","#ffceb8"],["海蓝","#cceeff","#dddefe"],["森林","#d8efca","#8fc9a8"]];
  const stamps=["✨","🌸","🌈","⭐","💗","☁️","🦋","🎀"];
  let pose=0,bg=0,stampSet=["✨","🌸"],title="白白的今天";
  $("#scr-design").innerHTML=`<button class="studioBack" id="studioBack">← 创意工坊</button><div class="card designIntro"><b>🖼️ 白白卡片工坊</b><span>白白只占卡片的一半，剩下的空间由你布置。成品保存到作品册，不会替换白白。</span><div class="designClock">${S.testMode?'🧪 测试模式：不限时':`还剩 <strong id="designClock">${Math.floor(left/60)}:${String(left%60).padStart(2,"0")}</strong>`}</div></div><div class="card cardMaker"><canvas id="cardCanvas" width="320" height="400"></canvas><div class="makerLabel">选择白白姿势</div><div class="makerPoses">${poses.map((p,i)=>`<button data-card-pose="${i}" class="${i?'':'on'}"><img src="${p}"></button>`).join("")}</div><div class="makerLabel">选择背景</div><div class="makerBgs">${bgs.map((x,i)=>`<button data-card-bg="${i}" class="${i?'':'on'}" style="--a:${x[1]};--b:${x[2]}">${x[0]}</button>`).join("")}</div><div class="makerLabel">添加小装饰（可多选）</div><div class="makerStamps">${stamps.map((x,i)=>`<button data-card-stamp="${x}" class="${i<2?'on':''}">${x}</button>`).join("")}</div><label class="roleName">卡片标题<input id="cardTitle" maxlength="10" value="${title}" autocomplete="off"></label><button class="btn wide" id="saveCard">💖 保存到我的作品</button><div class="cardGallery" id="cardGallery"></div></div>`;
  const canvas=$("#cardCanvas"),ctx=canvas.getContext("2d"),img=new Image();
  const gallery=()=>{$("#cardGallery").innerHTML=(S.designCards||[]).slice(-4).reverse().map((x,i)=>`<img src="${x.art}" alt="作品${i+1}">`).join("")};
  const paint=()=>{const g=ctx.createLinearGradient(0,0,320,400);g.addColorStop(0,bgs[bg][1]);g.addColorStop(1,bgs[bg][2]);ctx.fillStyle=g;ctx.fillRect(0,0,320,400);ctx.fillStyle="rgba(255,255,255,.38)";for(let i=0;i<18;i++){ctx.beginPath();ctx.arc((i*73)%330,(i*47)%390,2+(i%4),0,Math.PI*2);ctx.fill()}img.onload=()=>{const s=Math.min(230/img.width,225/img.height),w=img.width*s,h=img.height*s;ctx.drawImage(img,(320-w)/2,98+(225-h)/2,w,h);ctx.strokeStyle="rgba(255,255,255,.9)";ctx.lineWidth=8;ctx.strokeRect(8,8,304,384);ctx.strokeStyle="rgba(118,80,149,.25)";ctx.lineWidth=2;ctx.strokeRect(14,14,292,372);ctx.font="30px sans-serif";stampSet.forEach((x,i)=>ctx.fillText(x,i%2?255:28,70+Math.floor(i/2)*55));ctx.fillStyle=bg===2?"#fff":"#70517f";ctx.textAlign="center";ctx.font="bold 20px sans-serif";ctx.fillText(title||"白白的今天",160,365)};img.src=poses[pose]};
  $$('[data-card-pose]').forEach(b=>b.onclick=()=>{pose=+b.dataset.cardPose;$$('[data-card-pose]').forEach(x=>x.classList.toggle('on',x===b));paint()});
  $$('[data-card-bg]').forEach(b=>b.onclick=()=>{bg=+b.dataset.cardBg;$$('[data-card-bg]').forEach(x=>x.classList.toggle('on',x===b));paint()});
  $$('[data-card-stamp]').forEach(b=>b.onclick=()=>{const x=b.dataset.cardStamp,i=stampSet.indexOf(x);i>=0?stampSet.splice(i,1):stampSet.length<6&&stampSet.push(x);b.classList.toggle('on',stampSet.includes(x));paint()});
  $("#cardTitle").oninput=e=>{title=e.target.value;paint()};
  $("#saveCard").onclick=()=>{S.designCards=S.designCards||[];S.designCards.push({date:todayStr(),title:title||"白白的今天",art:canvas.toDataURL("image/jpeg",.86)});if(S.designCards.length>12)S.designCards.shift();save();gallery();confetti();sndWin();toast("💖 卡片已保存到我的作品")};
  $("#studioBack").onclick=()=>{designMode="";renderDesignStudio()};show("design","🎨 设计工坊");paint();gallery();startDesignTimer(left,()=>save());
}
function renderDesignStudio() {
  if(designTimer){clearInterval(designTimer);designTimer=null;}
  if(designSessionSave){designSessionSave();designSessionSave=null;}
  const legacyIds=(S.pet.customOutfits||[]).map(x=>x.id).filter(Boolean);
  ...
  const renderWorkbench = () => {
    ...
    $("#designOutfit").innerHTML=`<div class="designStage"><div class="petFig">${`<img class="petImg" src="${petVisual()}" alt="${esc(petName())}">`}<span class="petDeco" style="z-index:3;left:${shape.pos.x}%;top:${shape.pos.y}%;width:${Math.round(280*shape.base)}px;transform:translate(-50%,-50%)">${outfitVisual(preview)}</span></div></div>
      ...
      <div class="designPromise"><b>这不是普通涂色：</b>选好颜色后，装扮会立刻穿到白白身上，并永久放进衣橱。第一件免费，之后每件 🪙30。</div>
      <button class="btn wide" id="makeDesign">✨ 做好并穿到白白身上${(S.pet.customOutfits||[]).length?'（🪙30）':'（首次免费）'}</button>`;
  ...
  $("#drawHint").textContent="默认可拖动 · 选画笔后再画";
  const ctx=canvas.getContext("2d"),history=[],redo=[];let drawing=false,panning=false,color="#7b4b2a",size=3,tool="pan",filled=false,erasing=false,hasInk=false,start=null,shapeBase=null,offset={x:0,y:0},zoom=1,panStart=null;
  const clampView=()=>{const w=board.clientWidth||330,h=board.clientHeight||330;offset.x=Math.min(0,Math.max(w-900*zoom,offset.x));offset.y=Math.min(0,Math.max(h-900*zoom,offset.y));};
  const placeView=()=>{clampView();world.style.transform=`translate(${offset.x}px,${offset.y}px) scale(${zoom})`;board.classList.toggle("is-panning",tool==="pan")};
  const centerView=()=>{const w=board.clientWidth||330,h=board.clientHeight||330;offset={x:(w-900*zoom)/2,y:(h-900*zoom)/2};placeView()};
  centerView();
  const hideHint=()=>{if($("#drawHint"))$("#drawHint").style.display="none"};
  const snapshot=()=>{history.push(ctx.getImageData(0,0,900,900));if(history.length>8)history.shift();redo.length=0};
  const toolTip=()=>{const names={pan:"拖动画布",free:"曲线",line:"直线",arrow:"箭头",circle:"圆形",rect:"矩形",triangle:"三角形",diamond:"菱形",pentagon:"五边形",hexagon:"六边形",star:"五角星",heart:"爱心",cloud:"云朵",bubble:"对话框"};$("#drawTip").textContent=tool==="pan"?"现在是：拖动画布 · 不会留下笔迹":erasing?"现在是：橡皮 · 在画错的地方轻轻擦":"现在是："+(size===3?"细铅笔":size===6?"彩色笔":"粗画笔")+"画"+names[tool]+(filled&&!["free","line","arrow"].includes(tool)?"（实心）":"")+" · 按住拖动就能画"};
  if(S.designDraft&&S.designDraft.canvas){const im=new Image();im.onload=()=>{ctx.drawImage(im,0,0,900,900);hasInk=true;hideHint()};im.src=S.designDraft.canvas;if(S.designDraft.view){offset=S.designDraft.view.offset||offset;zoom=S.designDraft.view.zoom||1;placeView()}}
  ...
  const poly=(cx,cy,rx,ry,n,turn=-Math.PI/2)=>{for(let i=0;i<n;i++){const a=turn+i*Math.PI*2/n,x=cx+Math.cos(a)*rx,y=cy+Math.sin(a)*ry;i?ctx.lineTo(x,y):ctx.moveTo(x,y)}ctx.closePath()};
  const drawShape=(p)=>{const x=Math.min(start.x,p.x),y=Math.min(start.y,p.y),w=Math.abs(p.x-start.x),h=Math.abs(p.y-start.y),cx=x+w/2,cy=y+h/2;ctx.beginPath();if(tool==="line"){ctx.moveTo(start.x,start.y);ctx.lineTo(p.x,p.y)}else if(tool==="arrow"){const a=Math.atan2(p.y-start.y,p.x-start.x),head=Math.min(24,Math.max(12,Math.hypot(p.x-start.x,p.y-start.y)/4));ctx.moveTo(start.x,start.y);ctx.lineTo(p.x,p.y);ctx.moveTo(p.x,p.y);ctx.lineTo(p.x-Math.cos(a-.55)*head,p.y-Math.sin(a-.55)*head);ctx.moveTo(p.x,p.y);ctx.lineTo(p.x-Math.cos(a+.55)*head,p.y-Math.sin(a+.55)*head)}else if(tool==="rect"){ctx.rect(x,y,w,h)}else if(tool==="circle"){ctx.ellipse(cx,cy,w/2,h/2,0,0,Math.PI*2)}else if(tool==="triangle")poly(cx,cy,w/2,h/2,3);else if(tool==="diamond")poly(cx,cy,w/2,h/2,4);else if(tool==="pentagon")poly(cx,cy,w/2,h/2,5);else if(tool==="hexagon")poly(cx,cy,w/2,h/2,6);else if(tool==="star"){for(let i=0;i<10;i++){const a=-Math.PI/2+i*Math.PI/5,r=i%2?.42:1,px=cx+Math.cos(a)*w/2*r,py=cy+Math.sin(a)*h/2*r;i?ctx.lineTo(px,py):ctx.moveTo(px,py)}ctx.closePath()}else if(tool==="heart"){ctx.moveTo(cx,cy+h*.42);ctx.bezierCurveTo(x-w*.08,y+h*.62,x+w*.02,y+h*.08,cx,y+h*.28);ctx.bezierCurveTo(x+w*.98,y+h*.08,x+w*1.08,y+h*.62,cx,cy+h*.42);ctx.closePath()}else if(tool==="cloud"){ctx.moveTo(x+w*.15,y+h*.72);ctx.bezierCurveTo(x-w*.05,y+h*.68,x,y+h*.36,x+w*.23,y+h*.38);ctx.bezierCurveTo(x+w*.28,y+h*.05,x+w*.62,y,x+w*.7,y+h*.3);ctx.bezierCurveTo(x+w*1.02,y+h*.25,x+w*1.1,y+h*.67,x+w*.84,y+h*.74);ctx.closePath()}else if(tool==="bubble"){const r=Math.min(18,w/5,h/5);ctx.moveTo(x+r,y);ctx.arcTo(x+w,y,x+w,y+h,r);ctx.arcTo(x+w,y+h,x,y+h,r);ctx.lineTo(x+w*.38,y+h);ctx.lineTo(x+w*.25,y+h+h*.25);ctx.lineTo(x+w*.22,y+h);ctx.arcTo(x,y+h,x,y,r);ctx.arcTo(x,y,x+w,y,r);ctx.closePath()}if(filled&&!["line","arrow"].includes(tool))ctx.fill();ctx.stroke()};
  const begin=e=>{if(tool==="pan"){panning=true;panStart={x:e.clientX-offset.x,y:e.clientY-offset.y};if(canvas.setPointerCapture&&e.pointerId!==undefined)canvas.setPointerCapture(e.pointerId);e.preventDefault();return}snapshot();drawing=true;hasInk=true;hideHint();start=point(e);shapeBase=ctx.getImageData(0,0,900,900);ctx.globalCompositeOperation=erasing?"destination-out":"source-over";ctx.strokeStyle=color;ctx.fillStyle=color;ctx.lineWidth=erasing?18:size;ctx.lineCap="round";ctx.lineJoin="round";if(erasing||tool==="free"){ctx.beginPath();ctx.arc(start.x,start.y,ctx.lineWidth/2,0,Math.PI*2);ctx.fill();ctx.beginPath();ctx.moveTo(start.x,start.y)}e.preventDefault()};
  const move=e=>{if(panning){offset.x=e.clientX-panStart.x;offset.y=e.clientY-panStart.y;placeView();e.preventDefault();return}if(!drawing)return;const p=point(e);if(erasing||tool==="free"){ctx.lineTo(p.x,p.y);ctx.stroke()}else{ctx.putImageData(shapeBase,0,0);drawShape(p)}e.preventDefault()};
  const end=e=>{drawing=false;panning=false;if(canvas.releasePointerCapture&&e&&e.pointerId!==undefined&&canvas.hasPointerCapture&&canvas.hasPointerCapture(e.pointerId))canvas.releasePointerCapture(e.pointerId)};
  ...
  const chooseDrawTool=()=>{tool="free";$$(".drawShape").forEach(x=>x.classList.toggle("on",x.dataset.tool==="free"))};
  const chooseColor=(c,b)=>{color=c;erasing=false;chooseDrawTool();$$(".drawColor").forEach(x=>x.classList.toggle("on",x===b));$("#drawEraser").classList.remove("on");toolTip()};
  $$(".drawColor").forEach(b=>b.onclick=()=>chooseColor(b.dataset.color,b));
  $("#drawCustomColor").oninput=e=>chooseColor(e.target.value,null);
  $$(".drawSize").forEach(b=>b.onclick=()=>{size=+b.dataset.size;erasing=false;chooseDrawTool();$$(".drawSize").forEach(x=>x.classList.toggle("on",x===b));$("#drawEraser").classList.remove("on");toolTip()});
  $("#drawEraser").onclick=()=>{erasing=true;chooseDrawTool();$("#drawEraser").classList.add("on");toolTip()};
  $("#drawFill").onclick=()=>{filled=!filled;$("#drawFill").classList.toggle("on",filled);$("#drawFill").textContent="◉ 实心："+(filled?"开":"关");toolTip()};
  $$(".drawShape").forEach(b=>b.onclick=()=>{tool=b.dataset.tool;erasing=false;$$(".drawShape").forEach(x=>x.classList.toggle("on",x===b));$("#drawEraser").classList.remove("on");toolTip()});
  const zoomView=next=>{const w=board.clientWidth||330,h=board.clientHeight||330,cx=(w/2-offset.x)/zoom,cy=(h/2-offset.y)/zoom;zoom=Math.max(.45,Math.min(2.25,next));offset={x:w/2-cx*zoom,y:h/2-cy*zoom};placeView()};
  $("#drawZoomOut").onclick=()=>zoomView(zoom-.2);$("#drawZoomIn").onclick=()=>zoomView(zoom+.2);$("#drawCenter").onclick=()=>{zoom=1;centerView()};
  $$('[data-starter]').forEach(b=>b.onclick=()=>{snapshot();const im=new Image();im.onload=()=>{ctx.globalCompositeOperation="source-over";ctx.clearRect(0,0,900,900);const scale=Math.min(430/im.width,430/im.height),w=im.width*scale,h=im.height*scale;ctx.drawImage(im,(900-w)/2,(900-h)/2,w,h);hasInk=true;hideHint();zoom=1;centerView();toast("白白底稿放好啦，四周都留给你创作")};im.src=b.dataset.starter});
  ...
  startDesignTimer(left,designSessionSave);
}
function renderReward() {
  ...
  $("#toDesign").onclick = () => { designMode="";go(renderDesignStudio); };
